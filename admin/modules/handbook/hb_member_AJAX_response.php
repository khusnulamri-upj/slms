...
// query to database
$member_q = $dbs->query("SELECT m.member_id, m.member_name 
    FROM member AS m LEFT OUTER JOIN hb_mst_member_type AS h ON m.member_type_id = h.member_type_id
    WHERE (m.member_id LIKE '%$searchVal%' OR m.member_name LIKE '%$searchVal%') AND h.is_use_handbook = 1 LIMIT 10");
